...
CKheperaForaging::SFoodData::SFoodData() :
   HasFoodItem(false),
   FoodItemIdx(0),
   TotalFoodItems(0) {}

void CKheperaForaging::SFoodData::Reset() {
   HasFoodItem = false;
   FoodItemIdx = 0;
   TotalFoodItems = 0;
}
...
CKheperaForaging::SDiffusionParams::SDiffusionParams() :
   GoStraightAngleRange(CRadians(-1.0f), CRadians(1.0f)) {}

void CKheperaForaging::SDiffusionParams::Init(TConfigurationNode& t_node) {
   try {
...
void CKheperaForaging::SWheelTurningParams::Init(TConfigurationNode& t_node) {
   try {
...
CKheperaForaging::SStateData::SStateData() :
   ProbRange(0.0f, 1.0f) {}

void CKheperaForaging::SStateData::Init(TConfigurationNode& t_node) {
   try {
...
void CKheperaForaging::SStateData::Reset() {
   State = STATE_RESTING;
   InNest = true;
...
CKheperaForaging::CKheperaForaging() :
   m_pcWheels(NULL),
   m_pcLEDs(NULL),
   m_pcRABA(NULL),
   m_pcRABS(NULL),
...
void CKheperaForaging::Init(TConfigurationNode& t_node) {
   try {
      /*
...
void CKheperaForaging::UpdateState() {
   /* Reset state flags */
...
CVector2 CKheperaForaging::CalculateVectorToLight() {
...
CVector2 CKheperaForaging::DiffusionVector(bool& b_collision) {
...
void CKheperaForaging::ReturnToNest() {
 ...
 */
REGISTER_CONTROLLER(CKheperaForaging, "khepera_foraging_controller")

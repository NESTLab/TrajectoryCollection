#include "mixed_loop_functions.h"
...
CMixedLoopFunctions::CMixedLoopFunctions() :
...
void CMixedLoopFunctions::Init(TConfigurationNode& t_node) {
   try {
...
void CMixedLoopFunctions::Reset() {
   /* Zero the counters */
   m_unCollectedFood = 0;
   m_nEnergy = 0;
   /* Close the file */
...
void CMixedLoopFunctions::Destroy() {
   /* Close the file */
   m_cOutput.close();
}
...
CColor CMixedLoopFunctions::GetFloorColor(const CVector2& c_position_on_plane) {
...
void CMixedLoopFunctions::PreStep() {
   ...
   for(CSpace::TMapPerType::iterator it = m_cKheperas.begin();
      ...
      CKheperaMixed& cController = dynamic_cast<CKheperaMixed&>(cKhepera.GetControllableEntity().GetController());
...
REGISTER_LOOP_FUNCTIONS(CMixedLoopFunctions, "mixed_loop_functions")
